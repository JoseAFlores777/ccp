...
// chainYAML es autoBaseYAML (perfiles personal-cc/app-cc/emco-cc/personal-deepseek
// y reglas /work/personal→personal-cc, /work/emco→emco-cc) más un bloque
...
func TestChainAddNoTocaOtrosPrimarios(t *testing.T) {
	...
	if res.Primary != "personal-cc" {
		t.Fatalf("primario = %q, want personal-cc", res.Primary)
	}
	if !chainEq(res.AllowAdded, []string{"personal-deepseek"}) {
		t.Errorf("AllowAdded = %v, want [personal-deepseek]", res.AllowAdded)
	}

	if got := chainFallbackOf(t, home); !chainEq(got, []string{"app-cc", "emco-cc", "personal-deepseek"}) {
		t.Errorf("fallback = %v", got)
	}
	if got, _ := chainAllowOf(t, home, "personal-cc"); !chainEq(got, []string{"app-cc", "emco-cc", "personal-deepseek"}) {
		t.Errorf("allow_from[personal-cc] = %v", got)
	}
	...
	if got, _ := chainAllowOf(t, home, "emco-cc"); !chainEq(got, []string{"emco-cc"}) {
		t.Errorf("allow_from[emco-cc] = %v, want [emco-cc] intacto", got)
	}
}
...
func TestChainAddSinGateNoLoInventa(t *testing.T) {
	...
	if !chainEq(rc.Fallback, []string{"app-cc", "emco-cc", "personal-deepseek"}) {
...
func TestChainAddCreaLaEntradaEnDenyTotal(t *testing.T) {
	// El primario de /work/emco es emco-cc... pero usamos app-cc como primario
	// declarando una regla nueva: /work/labs no tiene entrada en allow_from.
	home := chainHome(t, chainYAML)
	if _, err := RuleSet(home, "/work/labs", "app-cc"); err != nil {
		t.Fatal(err)
	}
	...
	if res.Primary != "app-cc" {
		t.Fatalf("primario = %q, want app-cc", res.Primary)
	}
	if !res.AllowCreated {
		t.Error("AllowCreated debería ser true: no había entrada para app-cc")
	}
	got, ok := chainAllowOf(t, home, "app-cc")
	if !ok {
		t.Fatal("no se creó allow_from[app-cc]")
	}
	// Solo lo pedido: los otros candidatos siguen tan bloqueados como estaban.
	if !chainEq(got, []string{"personal-deepseek"}) {
		t.Errorf("allow_from[app-cc] = %v, want [personal-deepseek]", got)
	}
	...
	if !chainEq(rc.Denied, []string{"emco-cc"}) {
		t.Errorf("denegados = %v, want [emco-cc]", rc.Denied)
	}
}
...
func TestChainAddNoAllow(t *testing.T) {
	...
	if got, _ := chainAllowOf(t, home, "personal-cc"); !chainEq(got, []string{"app-cc", "emco-cc"}) {
		t.Errorf("allow_from[personal-cc] = %v, debía quedar intacto", got)
	}
...
func TestChainAddRechazaPerfilInexistente(t *testing.T) {
	...
	if got := chainFallbackOf(t, home); !chainEq(got, []string{"app-cc", "emco-cc"}) {
		t.Errorf("la cadena se mutó a medias: %v", got)
	}

	// Duplicado: tampoco entra dos veces.
	_, err = ChainAdd(home, ChainOpts{Cwd: "/work/personal"}, []string{"app-cc"})
...
func TestChainAddPosicion(t *testing.T) {
	...
	if got := chainFallbackOf(t, home); !chainEq(got, []string{"personal-deepseek", "app-cc", "emco-cc"}) {
...
func TestChainAddPrimarioAvisaQueEsImplicito(t *testing.T) {
	...
	res, err := ChainAdd(home, ChainOpts{Cwd: "/work/personal"}, []string{"personal-cc"})
	if err != nil {
	...
	if got := chainFallbackOf(t, home); !chainEq(got, []string{"app-cc", "emco-cc", "personal-cc"}) {
		t.Errorf("fallback = %v", got)
	}
	...
	if chainIndex(rc.Fallback, "personal-cc") >= 0 {
	...
	if got, _ := chainAllowOf(t, home, "personal-cc"); chainIndex(got, "personal-cc") >= 0 {
...
func TestChainMvReordena(t *testing.T) {
	...
	if got := chainFallbackOf(t, home); !chainEq(got, []string{"app-cc", "personal-deepseek", "emco-cc"}) {
		t.Errorf("fallback = %v", got)
	}
...
func TestChainRmAvisaSiNoEstaba(t *testing.T) {
	...
	msg := err.Error()
	if !strings.Contains(msg, "personal-deepseek") || !strings.Contains(msg, "app-cc") {
		t.Errorf("el error debe nombrar el perfil y lo que SÍ hay: %q", msg)
	}

	// El caso bueno, y con validación previa: `rm app-cc fantasma` no deja app-cc
	// fuera antes de descubrir que fantasma no estaba.
	if _, err := ChainRm(home, ChainOpts{Cwd: "/work/personal"}, []string{"app-cc", "fantasma"}); err == nil {
	...
	res, err := ChainRm(home, ChainOpts{Cwd: "/work/personal"}, []string{"app-cc"})
	if err != nil {
		t.Fatalf("ChainRm: %v", err)
	}
	if !chainEq(res.Removed, []string{"app-cc"}) {
		t.Errorf("Removed = %v", res.Removed)
	}
	if got := chainFallbackOf(t, home); !chainEq(got, []string{"emco-cc"}) {
		t.Errorf("fallback = %v", got)
	}
	...
	if !chainEq(res.AllowRemoved, []string{"app-cc"}) {
		t.Errorf("AllowRemoved = %v, want [app-cc]", res.AllowRemoved)
	}
	if got, _ := chainAllowOf(t, home, "personal-cc"); !chainEq(got, []string{"emco-cc"}) {
		t.Errorf("allow_from[personal-cc] = %v, want [emco-cc]", got)
	}
...
func TestChainRmCierraElCicloDeAdd(t *testing.T) {
	...
	allowAntes, _ := chainAllowOf(t, home, "personal-cc")

	if _, err := ChainAdd(home, opts, []string{"personal-deepseek"}); err != nil {
		t.Fatalf("ChainAdd: %v", err)
	}
	if got, _ := chainAllowOf(t, home, "personal-cc"); chainIndex(got, "personal-deepseek") < 0 {
	...
	if got, _ := chainAllowOf(t, home, "personal-cc"); !chainEq(got, allowAntes) {
		t.Errorf("allow_from[personal-cc] = %v, want %v", got, allowAntes)
	}
}
...
func TestChainRmNoAllow(t *testing.T) {
	...
	res, err := ChainRm(home, ChainOpts{Cwd: "/work/personal", NoAllow: true}, []string{"app-cc"})
	if err != nil {
		t.Fatalf("ChainRm: %v", err)
	}
...
func TestChainAddDesatascaElDenyTotal(t *testing.T) {
	home := chainHome(t, chainYAML)
	// /work/labs → app-cc, que NO tiene entrada en allow_from: deny total.
	if _, err := RuleSet(home, "/work/labs", "app-cc"); err != nil {
		t.Fatal(err)
	}
	...
	// emco-cc YA está en `fallback`. Antes esto era ChainErrDuplicate y el repo
	// se quedaba sin salida.
	res, err := ChainAdd(home, opts, []string{"emco-cc"})
	if err != nil {
		t.Fatalf("add de un perfil en la cadena pero denegado debe ABRIR el gate: %v", err)
	}
	if !res.AllowCreated || !chainEq(res.AllowAdded, []string{"emco-cc"}) {
	...
	if !chainEq(res.Fallback, []string{"app-cc", "emco-cc"}) {
		t.Errorf("la cadena no debía cambiar: %v", res.Fallback)
	}
	var vistaNota bool
	for _, n := range res.Notes {
		if n.Kind == ChainNoteAlreadyInChain && n.Profile == "emco-cc" {
			vistaNota = true
		}
	}
	if !vistaNota {
	...
	if !chainEq(rc.Fallback, []string{"emco-cc"}) {
		t.Errorf("cadena efectiva = %v, want [emco-cc]", rc.Fallback)
	}

	// Repetirlo ya SÍ es un duplicado: está en la cadena y además autorizado.
	if _, err := ChainAdd(home, opts, []string{"emco-cc"}); chainErrKind(t, err) != ChainErrDuplicate {
		t.Errorf("el segundo add debe ser duplicado: %v", err)
	}
	// Y con --no-allow tampoco hay gate que abrir, así que vuelve a ser duplicado.
	if _, err := ChainAdd(home, ChainOpts{Cwd: "/work/labs", NoAllow: true}, []string{"app-cc"}); err == nil {
...
func TestChainSetReemplaza(t *testing.T) {
	...
	res, err := ChainSet(home, ChainOpts{Cwd: "/work/personal"}, []string{"emco-cc", "personal-deepseek"})
	if err != nil {
		t.Fatalf("ChainSet: %v", err)
	}
	if !chainEq(res.Fallback, []string{"emco-cc", "personal-deepseek"}) {
		t.Errorf("Fallback = %v", res.Fallback)
	}
	if got := chainFallbackOf(t, home); !chainEq(got, []string{"emco-cc", "personal-deepseek"}) {
		t.Errorf("fallback en disco = %v", got)
	}
	// set ajusta el gate en las DOS direcciones, porque es el rm+add que de hecho
	// es: personal-deepseek entra y se autoriza, app-cc sale y pierde el permiso,
	// emco-cc sigue en ambos lados y no se toca. Sin esto, `set` podía dejar la
	...
	if got, _ := chainAllowOf(t, home, "personal-cc"); !chainEq(got, []string{"emco-cc", "personal-deepseek"}) {
		t.Errorf("allow_from[personal-cc] = %v", got)
	}
	...
	if !chainEq(rc.Fallback, []string{"emco-cc", "personal-deepseek"}) {
	...
	if _, err := ChainSet(home, ChainOpts{Cwd: "/work/personal"}, []string{"app-cc", "app-cc"}); chainErrKind(t, err) != ChainErrDuplicate {
...
func TestChainSinBloqueAutoHandoff(t *testing.T) {
	...
	home = chainHome(t, chainYAML)
	_, err = ChainAdd(home, ChainOpts{Cwd: "/work/personal", Policy: "noche"}, []string{"app-cc"})
