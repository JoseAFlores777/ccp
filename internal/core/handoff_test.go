...
func TestHandoffForwardCopiesAndMarks(t *testing.T) {
	...
	if len(h.Active) != 1 || h.Active[0].To != "emco-cc" || h.Active[0].Session != uuid {
...
func TestHandoffForwardPermiteSegundoActivo(t *testing.T) {
	...
	if _, err := HandoffForward(home, "personal-cc", "emco-cc", cwdA, uuidA, true, false, false, time.Now()); err != nil {
		t.Fatalf("primer forward: %v", err)
	}
	if _, err := HandoffForward(home, "personal-cc", "emco-cc", cwdB, uuidB, true, false, false, time.Now()); err != nil {
...
func TestHandoffForwardBloqueaSesionEnVuelo(t *testing.T) {
	...
	if _, err := HandoffForward(home, "personal-cc", "emco-cc", cwd, uuid, true, false, false, time.Now()); err != nil {
		t.Fatal(err)
	}
...
// (personal→emco→kimi): estando en el destino, re-prestar la sesión que llegó
...
func TestHandoffForwardBloqueaCadena(t *testing.T) {
	...
	// Marcador activo personal-cc → emco-cc en este repo, con ESA sesión.
	...
	// El forward copió el jsonl al destino con el mismo uuid: desde emco-cc la
	// sesión existe y se podría intentar re-prestar a un tercer perfil.
	writeJSONL(t, ProjectDir(home+"/profiles/emco-cc/cc-home", slug), uuid, "N", time.Now())
	_, err := HandoffForward(home, "emco-cc", "personal-cc", cwd, uuid, true, false, false, time.Now())
	if err == nil {
...
// por sesión — con un activo personal-cc → emco-cc en /repo, estando en emco-cc
// NINGUNA sesión de ese repo se podía prestar, ni las nacidas ya en emco-cc que
...
func TestHandoffForwardPermiteOtraSesionDelMismoRepo(t *testing.T) {
	...
	cfg := &Config{Version: SchemaVersion, Profiles: map[string]Profile{
		"personal-cc": {Type: "official"}, "emco-cc": {Type: "official"}, "kimi": {Type: "official"},
	}}
	if err := Save(home, cfg); err != nil {
		t.Fatal(err)
	}
	...
	// Activo personal-cc → emco-cc con la sesión `prestada`.
	if err := SaveHandoffs(home, &Handoffs{Version: HandoffsVersion, Active: []Marker{{
		Session: prestada, Slug: slug, Cwd: cwd, From: "personal-cc", To: "emco-cc",
		Since: "2026-07-25T00:00:00Z",
	}}}); err != nil {
		t.Fatal(err)
	}
	// `propia` nació en emco-cc: no vino de ningún handoff, no encadena.
	writeJSONL(t, ProjectDir(home+"/profiles/emco-cc/cc-home", slug), propia, "Propia", time.Now())

	if _, err := HandoffForward(home, "emco-cc", "kimi", cwd, propia, true, false, false, time.Now()); err != nil {
...
func TestHandoffForwardEmiteYolo(t *testing.T) {
	...
	emit, err := HandoffForward(home, "personal-cc", "emco-cc", cwd, uuid, false, true, false, time.Now())
	if err != nil {
		t.Fatal(err)
	}
	...
	emit2, err := HandoffForward(home, "personal-cc", "emco-cc", cwd, uuid, false, false, false, time.Now())
	if err != nil {
		t.Fatal(err)
	}
...
func TestHandoffForwardSameProfile(t *testing.T) {
	home := t.TempDir()
	seedHandoffEnv(t, home)
	if _, err := HandoffForward(home, "emco-cc", "emco-cc", "/repo", "u", true, false, false, time.Now()); err == nil {
...
func TestHandoffForwardUnknownFromProfile(t *testing.T) {
	home := t.TempDir()
	seedHandoffEnv(t, home)
	_, err := HandoffForward(home, "no-existe", "emco-cc", "/repo", "u", true, false, false, time.Now())
	if err == nil {
...
func TestHandoffForwardCrossProviderWarns(t *testing.T) {
	...
	t.Run("official→official sin warning", func(t *testing.T) {
		...
		emit, err := HandoffForward(home, "personal-cc", "emco-cc", cwd, uuid, false, false, false, time.Now())
		if err != nil {
			t.Fatal(err)
		}
