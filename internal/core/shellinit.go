...
const CompletionBash = `_ccp() {
  ...
  case "${COMP_WORDS[1]}" in
    ...
    desktop) [[ $COMP_CWORD -eq 2 ]] && COMPREPLY=( $(compgen -W "open list path prepare rm default $(ccp profile list 2>/dev/null)" -- "$cur") )
             [[ $COMP_CWORD -eq 3 && "${COMP_WORDS[2]}" =~ ^(open|path|prepare|rm)$ ]] && COMPREPLY=( $(compgen -W "default $(ccp profile list 2>/dev/null)" -- "$cur") ) ;;
...
_ccp() {
  ...
  case "${words[2]}" in
    ...
    desktop) (( CURRENT == 3 )) && compadd -- open list path prepare rm default ${(f)"$(ccp profile list 2>/dev/null)"}
             (( CURRENT == 4 )) && [[ "${words[3]}" =~ ^(open|path|prepare|rm)$ ]] && compadd -- default ${(f)"$(ccp profile list 2>/dev/null)"} ;;
