...
- **Profile scope is partial.** A `cc-home` symlinks `agents/ commands/ skills/` from global, so a profile cannot own those types. At profile scope only rule/hook (overlay-native) are writable; mcp is unsupported at profile (its real location, `$CLAUDE_CONFIG_DIR.json`, was not confirmed, so profile-mcp errors with guidance to use global or project); agent/command/skill are refused with guidance to use global (already shared to every profile) or project. The `cc-home` seeding model is left unchanged.
